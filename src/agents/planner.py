...
def _derive_requested_aspects(user_input: str, intent: QueryIntent, focus: str) -> tuple[str, ...]:
    lowered = str(user_input or "").strip().lower()
    aspects: list[str] = []
    inventory_like_question = bool(
        lowered.startswith(("what kind", "what are", "which "))
        or "what ec2" in lowered
        or "what instance" in lowered
        or "what instances" in lowered
        or "what workloads" in lowered
        or "what services" in lowered
        or "which instances" in lowered
        or "which workloads" in lowered
        or "which services" in lowered
    )

    if intent.mode == "incident_rca":
        aspects.append("diagnosis")
    if _looks_like_inventory_request(lowered) or inventory_like_question:
...
def _answer_markers_for_aspect(aspect: str) -> tuple[str, ...]:
    mapping = {
        "inventory": (
            "deploy",
            "statefulset",
        ...
        "capacity": ("cpu", "ram", "memory", "allocatable", "total", "vcpu", "gi", "mi"),
        "cost": ("cost", "monthly", "usd", "$", "spend", "price", "savings"),
        "optimization": ("optimiz", "improv", "recommend", "saving", "opportunit", "rightsize"),
        "explanation": ("step", "flow", "request", "reverse proxy", "load balancer", "ingress"),
        "diagnosis": ("issue summary", "severity", "recommended action", "root cause", "because"),
        "analysis": ("answer", "details", "what i verified"),
    }
    ...
    for aspect in aspects:
        ...
        objectives.append(
            PlanObjective(
                ...
                answer_markers=_answer_markers_for_aspect(aspect),
...
def build_turn_plan(
    *,
    ...
    allow_broad_discovery = (stage == "scope" or inventory_request) and not follow_up_action
